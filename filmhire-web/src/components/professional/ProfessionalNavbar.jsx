            ...
            <div key={item.id} className="relative group w-full">
              ...
              >
                <div className="relative">
                  <Icon className={`w-[18px] h-[18px] transition-transform duration-200 ${isActive ? "stroke-[2.2] scale-105" : "stroke-[1.8]"}`} />
                  
                  {/* Alert Notification Micro-Badge */}
                  {item.badge && (
                    <span className="absolute -top-1 -right-1 w-2 h-2 bg-white rounded-full ring-2 ring-[#09090b]" />
                  )}
                </div>

                {/* Left Highlight Rail Accent */}
                {isActive && (
                  <div className="absolute left-0 top-1/3 bottom-1/3 w-[3px] bg-white rounded-r-full" />
                )}
              </button>

              {/* FLOATING HOVER NAME DESCRIPTIONS */}
